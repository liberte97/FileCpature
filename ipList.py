...
Country = {"GH":"가나","GA":"가봉","GY":"가이아나","GM":"감비아","GP":"프랑스","GT":"과테말라","GU":"미국",
		   ...
		   "PL":"폴란드","PR":"미국","FR":"프랑스","GF":"프랑스","PF":"프랑스","FJ":"피지","FI":"필란드","PH":"필리핀","HU":"헝가리", "KR":"한국"}
...
def get_title(path):
	...
	for i in range(title_num):
		SiteName[i] = data.URL[i]
	return title_num, SiteName

if __name__ == "__main__":
	Num, SiteName = get_title('test.xlsx')
	with open("ipListFinal.txt", "w+") as file_w:
		for i in range(Num):
			print(SiteName[i])
			if SiteName[i] == "127.0.0.1" or SiteName[i] == "0.0.0.0":
				file_w.writelines(f"{SiteName[i]}\tNone\n")
			else:
				obj = IPWhois(SiteName[i])
				data = obj.lookup_whois()
				country = data['nets'][0]['country']
				file_w.writelines(f"{SiteName[i]}\t{country}\t{Country[country]}\n")
